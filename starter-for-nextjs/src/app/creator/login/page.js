...
export default function CreatorLogin() {
    ...
    const handleGoogleLogin = () => {
        try {
            account.createOAuth2Session(
                ...
                `${window.location.origin}/creator/login`
            );
        } catch (err) {
            console.error(err);
    ...
    const handleEmailLogin = async (e) => {
        ...
        try {
            ...
            // Tag user as creator via prefs
            try {
                await account.updatePrefs({ role: "creator" });
            } catch (prefError) {
        ...
        <div className="min-h-screen bg-white relative overflow-hidden flex items-center justify-center p-4">
            ...
            <div className="relative z-10 w-full max-w-md backdrop-blur-xl bg-white/60 border-2 border-white/50 shadow-[0_8px_32px_0_rgba(31,38,135,0.15)] rounded-3xl p-6 md:p-10 flex flex-col gap-6">
                <div className="text-center space-y-2">
                    <h2 className="text-3xl font-black uppercase italic tracking-tighter">Creator Login</h2>
